<?php
$db = get_db();
$vraElements = $db->getTable('VraCoreElement')->findBy(array('record_type' => 'Item',
                                                'record_id'   => $item->id,
                                                'vra_element_id' => false,
                                    ));

$recordAttributes = $db->getTable('VraCoreAttribute')->findBy(array('record_type' => 'Item',
                                                'record_id'   => $item->id,
                                                'vra_element_id' => false,
                                                'element_id' => false,
                                    ));
...
$recordHref = record_url($item, 'show', true);
$recordId = 'w_' . $item->id;
$recordRefid = $item->id;
   ...
   >
        <?php foreach($vraElementSets as $elementKey => $vraElementSet): ?>
            <<?php echo lcfirst($elementKey) . 'Set'; ?>>
                ...
                ?>
                <display><?php echo metadata($item, array('VRA Core', $elementKey)); ?></display>
