<?php
$db = get_db();
$vraElements = $db->getTable('VraCoreElement')->findBy(array('record_type' => 'File',
                                                'record_id'   => $file->id,
                                                'vra_element_id' => false,
                                    ));

$recordAttributes = $db->getTable('VraCoreAttribute')->findBy(array('record_type' => 'File',
                                                'record_id'   => $file->id,
                                                'vra_element_id' => false,
                                                'element_id' => false,
                                    ));
...
$recordHref = file_display_url($file, 'original');
$recordId = 'i_' . $file->id;
$recordRefid = $file->id;
    ...
    >
        <?php foreach($vraElementSets as $elementKey => $vraElementSet): ?>
            <<?php echo lcfirst($elementKey) . 'Set'; ?>>
                ...
                ?>
                <display><?php echo metadata($file, array('VRA Core', $elementKey)); ?></display>
